...
public class FCFS implements ActionListener{
	...
	private JFrame resultsFrame = new JFrame("First Come First Serve Results");
	...
	private JFrame ganttFrame = new JFrame("First Come First Serve Gantt Chart");
	
	int[] intFinalTime, intWaitingTime, intTurnAroundTime, intArrivalTimeCopy, intBurstTimeCopy, intPriorityCopy;
	int intJobs;
	double dblWTSum = 0, dblTATSum = 0;
	double dblWaitingTimeAve = 0, dblTurnAroundTimeAve = 0;
	String[] strLetters;
	
	public void FCFSCompute(String[] strLetters, int[] intArrivalTime, int intBurstTime[], int intPriority[], int intJobs){
		...
		//FCFS
		intFinalTime[0] = intArrivalTime[0] + intBurstTime[0];
		
		for(int intCounter = 0; intCounter < intJobs; intCounter++){
			if(intCounter == 0){
				intWaitingTime[intCounter] = 0;
			} else {
				if(intFinalTime[intCounter - 1] < intArrivalTime[intCounter]){
					intFinalTime[intCounter] = intArrivalTime[intCounter] + intBurstTime[intCounter];
					intWaitingTime[intCounter] = 0;
				} else {
					intFinalTime[intCounter] = intFinalTime[intCounter - 1] + intBurstTime[intCounter];
					intWaitingTime[intCounter] = intFinalTime[intCounter - 1] - intArrivalTime[intCounter];
				}
			}
			intTurnAroundTime[intCounter] = intWaitingTime[intCounter] + intBurstTime[intCounter];
