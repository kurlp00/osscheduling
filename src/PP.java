...
public class PP implements ActionListener{
	...
	private JFrame resultsFrame = new JFrame("Preemptive Priority Results");
	...
	private JFrame ganttFrame = new JFrame("Preemptive Priority Gantt Chart");
	
	int[] intFinalTime, intWaitingTime, intTurnAroundTime, intRemainTime, intFinalTimeCopy;
	int intJobs;
	...
	public void PPCompute(String[] strLetters, int[] intArrivalTime, int intBurstTime[], int intPriority[], int intJobs){
		...
		intFinalTimeCopy = new int[1000];
		intRemainTime = new int[intJobs];
		...
		for(int intCounter = 0; intCounter < intJobs; intCounter++){
			intRemainTime[intCounter] = intBurstTime[intCounter];
		}
		
		int intFinalTimeCounter = 0;
		...
		//PP
		intPriority[14] = 11;
		for(intTime = 0; intRemain != 0; intTime++){
			intSmallest = 14;
		    for(int intCounter = 0; intCounter < intJobs; intCounter++){
		    	if(intArrivalTime[intCounter] <= intTime && intPriority[intCounter] < intPriority[intSmallest] && intRemainTime[intCounter] > 0){
		    		intSmallest = intCounter;
		    	}
		    }
		    strSeq += strLetters[intSmallest];
		    intRemainTime[intSmallest]--;
		    if(intRemainTime[intSmallest] == 0)
		    {
		    	intRemain--;
		    	intFinalTime[intSmallest] = intTime + 1;
		    	intTurnAroundTime[intSmallest] = (intTime + 1) - intArrivalTime[intSmallest];
			    intWaitingTime[intSmallest] = (intTime + 1) - intArrivalTime[intSmallest] - intBurstTime[intSmallest];
		...
		for(int intCounter = 0; intCounter < intFinalTimeCounter; intCounter++){
			System.out.println(intFinalTimeCopy[intFinalTimeCounter]);
		}
